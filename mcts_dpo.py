...
@lru_cache(1024) #동일한 입력값에 대한 반복계산 x
def extract_label(text: str) -> str:

    if '\n####' in text: #줄바뀔때
        text = text.split('\n####')[-1].replace(',','')
    elif 'The answer is' in text: #답변시
        text = text.split('The answer is')[-1].replace(',','')
    numbers = pattern.findall(text) 
  #예시 위의 정규식 사용
  # pattern.findall("The temperature is -23.45 degrees.") → ['-23.45']
  #pattern.findall("Scores are 12, 45.67, and -89.") → ['12', '45.67', '-89']
    if not numbers:
        return None
    return numbers[0]


def check(gt,ans): #주어진 정답, 생성한 답변 (ground truth <-> answe)
...
def get_node_id(answers,ans): #answers 리스트중에 특정 ans 인덱스 반환
    return answers.index(ans)

def get_oldest_father(answers,ans,childs): #주어진 답변의 가능한 부모 노드들을 찾고, 그 중 가장 오래된(인덱스가 작은) 부모 노드를 반환
    possible_fathers = []
